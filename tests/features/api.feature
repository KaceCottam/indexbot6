# TODO more tests needed for mutation testing success
Feature: The api can be successfully interacted with.
  ...
  Scenario: We can create a database.
    ...
    Then there are no users who subscribe to role "1" in guild "1"
    But there are no roles that user "1" in guild "1" subscribes to
    But there are no roles in guild "1"

  Scenario: We can insert users into the database.
    Given an empty initial database
    When inserting user "1" in guild "1" to role "1"
    Then the users who subscribe to role "1" in guild "1" is 1
    But the roles that user "1" in guild "1" has is 1
    But the roles that guild "1" has is 1
  ...
  Scenario: We can query a database to find many users.
    ...
    Then the users who subscribe to role "1" in guild "1" is 1
    But the users who subscribe to role "1" in guild "0" is 1
    But the roles that user "1" in guild "0" has is 1
    But the roles that user "1" in guild "1" has is 1
    But the roles that guild "1" has is 1
    But the roles that guild "0" has is 1
  ...
  Scenario: We can remove users from a role in a database.
    ...
    But there are no users who subscribe to role "1" in guild "1"
    But there are no roles that user "1" in guild "1" subscribes to
    But there are no roles in guild "1"
