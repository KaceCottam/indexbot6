...
def removeUserFromRole(db: TinyDB, guildid: str, roleid: str, userid: str) -> dict | None:
    ...
    if user := db.get(query):
        db.remove(query)
        return user
    return None


def removeRole(db: TinyDB, guildid: str, roleid: str) -> list[dict] | None:
    """
    Removes a role from a guild
    :param db: database
    :param guildid: guild id
    :param roleid: role id
    :return: the role that was removed if it exists, else None
    """
    query = guild(guildid) & role(roleid)
    if roles := db.search(query):
        db.remove(query)
        return roles
    return None


def removeUser(db: TinyDB, guildid: str, userid: str) -> list[dict] | None:
    """
    Removes a role from a guild
    :param db: database
    :param guildid: guild id
...
def removeGuild(db: TinyDB, guildid: str) -> list[dict] | None:
    ...
    :param db: database
    :param guildid: guild id
    :return: the guild that was removed if it exists, else None⌈
    """
    query = guild(guildid)
    if guild := db.search(query):
        db.remove(query)
        return guild
    return None
